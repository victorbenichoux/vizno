...
class MatplotlibRenderer(
    GenericContentRenderer[Figure, MatplotlibContentConfiguration]
):
    @staticmethod
    def render(
        figure,
        ...
        with io.StringIO() as str_io:
            figure.savefig(
                str_io, format="svg", transparent=True, bbox_inches=bbox_inches
            )
            svg = str_io.getvalue()
            ...
            for element in root.iter("{http://www.w3.org/2000/svg}text"):
                ...
                if s:
                    rules = {
                        r[0]: r[1]
                        for r in map(
                            lambda x: x.split(":") if x else None, s.split(";")
                        )
                        if r
                    }
